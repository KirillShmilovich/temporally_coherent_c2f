...
class OpenMMEnergy(Energy):
    def __init__(self, dimension, pdb_fname, mol_name="ADP", **kwargs):
        ...
        if mol_name == "ADP":
            ...
            charmm_tops_fname = kwargs["tops_fname"]
            top_files = [str(f) for f in Path(charmm_tops_fname).glob("*prm")]
            top_files += [str(f) for f in Path(charmm_tops_fname).glob("*rtf")]
            psf = app.CharmmPsfFile(str(list(Path(charmm_tops_fname).glob("*psf"))[0]))
            params = app.CharmmParameterSet(*top_files)
            self.system = psf.createSystem(params, nonbondedMethod=app.NoCutoff)
